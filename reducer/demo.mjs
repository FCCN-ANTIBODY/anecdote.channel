...
// the merge-only ratchet collapses the duplicates — the aggregate-by-question collision,
// with a trust-weighted gatherer count falling out for free. The coda shows the local
// cache: a dictionary saved to a domain-scoped store and cold-loaded by re-deriving vectors.
//
//   node reducer/demo.mjs
//
...
// Local cache: persist the collided dictionary to a domain-scoped store, then cold-load it by
// re-deriving every vector from its durable name — no floats on disk, nothing left in memory.
const store = memoryStore();           // browser: idbStore(), partitioned per origin
await U.save(store);
const cold = await Reducer.load(store, "anecdote:dictionary", { embed: toyEmbed, name: fewestVerbs });
console.log(
  `\nlocal cache: saved ${U.labels.length} labels, cold-loaded ${cold.labels.length} by re-deriving ` +
  `vectors from names\n(persisted bytes carry no embeddings — the name is authoritative, the vector is derived).`
);
