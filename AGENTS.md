...
| `press/` (`halt` · `bench` · `counter` · `overlay`) | **The total experience, four ways, over one engine** (`press/press.mjs`): one page that feels like one place while every view is a frame onto another origin. A bottle's README shown as its index (`index.md` wins; unbuilt Jekyll rendered from source by `jekyll-enough`, missing pieces *named*), always in a sterile `data:` chamber; a statement with an object attached, signed and printed into a pile as a commit, pointing at the *version* it was said over; a bottle caught through a floor on **another origin**, opened sterile, kept only by an explicit act. Needs the subdomain world: `node scripts/serve-constellation.mjs` → `http://anecdote.localhost:8137/press/`. See `press/README.md` |
| `press/broadcast.html` | **The target: a page that does nothing but broadcast.** A bottle cut three ways — one still code, a collage of n at once, or an endless stream — each reporting the FLOOR of screen-reads a perfect camera would need, so two phones pointed at each other produce a yield instead of an impression (`docs/preview-paradigm.md`) |
